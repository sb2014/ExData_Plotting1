...
with (subDat, {
    ...
    plot(as.POSIXct(paste(subDat$Date, subDat$Time), format="%Y-%m-%d %H:%M:%S"), as.numeric(as.character(subDat$Sub_metering_1)), ylab="Global Active Power (kilowatts)", xlab="", type="l")
    lines(as.POSIXct(paste(subDat$Date, subDat$Time), format="%Y-%m-%d %H:%M:%S"), as.numeric(as.character(subDat$Sub_metering_2)), ylab="Global Active Power (kilowatts)", xlab="", type="l", col="red")
    lines(as.POSIXct(paste(subDat$Date, subDat$Time), format="%Y-%m-%d %H:%M:%S"), as.numeric(as.character(subDat$Sub_metering_3)), ylab="Global Active Power (kilowatts)", xlab="", type="l", col="blue")
